...
sealed interface ProposedAction {
    ...
    data class ReplyNotification(val key: String, val text: String) : ProposedAction { override val summary = "Reply to notification: $text" }
...
fun riskOf(action: ProposedAction): RiskLevel = when (action) {
    ...
    is ProposedAction.CreateEvent, is ProposedAction.ReplyNotification -> RiskLevel.HIGH
 ...
 *
 * USER is something the person typed or spoke. AGENT is the assistant executing
 * a goal the user asked for (goal-runner work — trusted like USER's intent, but
 * executed unattended, so it must respect the auto-execute ceiling). BACKGROUND
 * is machine-generated text — notification bodies, gallery/contact scan output,
 * OCR'd screen content — which is untrusted input that happens to be phrased as
 * a request. A filename or message body can say "delete file /x"; that must
 * never carry the authority of the user saying it.
 */
enum class ActionOrigin { USER, AGENT, BACKGROUND }
