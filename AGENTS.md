...
- `docs/TRACKS.md` — the current five-track split for parallel agents: what each track owns and may not touch, the wave ramp (five agents cannot start today — Gate 0 and the god-object decomposition must be serialized first), the cross-cutting tenancy and computer-use waves, the shared-file conflict surface, and the inter-track interface contract.
