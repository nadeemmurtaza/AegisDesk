...
- `docs/TENANCY_DESIGN.md` — the multi-tenancy model: three tiers (on-device workspaces, organization fleets, hosted — the last costed and not recommended), isolation by per-tenant key rather than `tenant_id` filtering, what must change in the code (the 8 process-wide holders and the database singleton), tenant lifecycle, an honest analysis of what on-device tenancy does *not* protect against, the new UI routes, and the T-1…T-8 slice table.
