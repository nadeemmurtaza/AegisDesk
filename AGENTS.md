...
- `docs/ENGINEERING.md` — **start here before writing code.** Part A = the complete delivery slice list (0–24) with dependencies, gates, and status, headed by Gate 0 (the build has never been green — nothing is verifiable until it is). Part B = the engineering standard: module rules, Kotlin/KMP/Compose practice, accessibility as a definition-of-done, the security threat model and how authentication must bind to hardware-backed keys rather than a boolean, performance budgets, the testing pyramid (incl. property-testing the policy engine), supply-chain and release gates, what "enterprise" may and may not override, working agreements, and the repo's known anti-patterns.
- `docs/TENANCY_DESIGN.md` — tenancy, identity and multi-device: the Organization → Person → Profile (Work/Personal) → Device model, isolation by per-profile key rather than `tenant_id` filtering, multi-device as enrollment over the existing SAS-verified channel rather than a server login, org governance bounded to tighten-only with Personal cryptographically out of reach, the recovery-kit trade, per-platform key custody tiers (Windows DPAPI is not hardware-backed), what on-device isolation does *not* protect against, the code changes required (the 8 holders + the database singleton), new UI routes, and the T-1…T-12 slice table.
- `docs/TRACKS.md` — the current five-track split for parallel agents: what each track owns and may not touch, the wave ramp (Gate 0 is cleared, so all five can start; only Track 3's god-object decomposition stays serialized inside `apps/android`), the cross-cutting tenancy and computer-use waves, the shared-file conflict surface, and the inter-track interface contract. It is the map — the per-track briefs below are what an assigned agent actually works from.
- `docs/tracks/T1-build-ci-release.md` — Track 1's complete brief: the version catalog (AGP and Kotlin are each declared in three files today), the no-inline-version guard, CODEOWNERS with the `apps/android` UI-vs-agents split, the lint guard table, and supply chain (dependency verification, SBOM, reproducible builds).
- `docs/tracks/T2-core-data-policy.md` — Track 2's complete brief: the `instrumented-tests` failure with the three eliminations already made, unifying the three risk vocabularies, property tests over the policy engine, conversation persistence, streaming, and tenancy T-1/T-2/T-3.
- `docs/tracks/T3-design-system-android-ui.md` — Track 3's complete brief: the god-object decomposition (done first and alone, because it blocks four tracks), string externalization, unpinning dark theme, the ~60-component library with accessibility in the first commit, and the 105-route tree.
- `docs/tracks/T4-platform-bodies.md` — Track 4's complete brief: the Mac/Windows hardware prerequisite and what Linux can and cannot verify, desktop chat parity, the expanded three-pane layout and shortcuts, Windows TPM custody (DPAPI is not hardware-backed), the iOS body, and multi-device enrollment.
- `docs/tracks/T5-agents-automation-safety.md` — Track 5's complete brief: the two invariants (the router never decides permission; no agent holds execution authority), the prompt-injection corpus built before the capability, the untrusted-screen boundary, consequence classes and their floors, pre-flight verification and semantic targeting, the sandbox and concurrency gaps, and per-profile agent memory.
- `docs/PARALLEL_RULES.md` — **read before starting work alongside other agents.** The general conflict-avoidance rule set: why nothing is enforceable until the build is green, what cannot be parallelised (schema migrations, the god-object decomposition, anything renaming a widely-used symbol), one-owner-per-file, one-declaration-per-fact, one-name-per-concept, surgical renames, no half-wired interfaces, docs and code in the same commit, and a guard table showing which checks exist today versus which still need building. Every example is a real defect from this repo. `docs/PARALLEL_WORKSPLIT.md` is a specific four-body split; this is the general rule set.
- `docs/tracks/KICKOFF.md` — copy-pasteable starting prompts for the five track agents, each with owned paths, never-touch list, proven setup commands, the first slice in detail, verification commands and PR rules; plus a per-track readiness table and the precondition that blocks all of them (the fixes and briefs live on a branch, not `main`).
- `docs/UNWIRED.md` — an inventory of code that compiles and is never reached: ~24 dev-console inspector backends against 5 tabs (only DevLogger wired), the whole `engine/ai` package including hallucination detection, `ExecutionGuard.checkWithContext` (the C-3 pre-flight guard, written and called from nowhere), `ModelProvider.stream()` (tested, no production caller), 36 NotImplementedError capability stubs on iOS/macOS, and the registry/manager/learning objects needing a wire-it-or-delete-it decision. Records the scans' false positives too — manifest-declared components are invisible to a source scan.
- `docs/AUTH_DESIGN.md` — device and user authentication: why an offline-first app with no server cannot have a login that checks a credential (the credential must derive the key, or an attacker with the device skips the check entirely), the two halves (enrolled Ed25519 device identity + passphrase/biometric) and why neither alone opens a profile, the AuthStrength ladder bound to PolicyMode with floors that raise and cannot lower, AuthProof replacing the forgeable `biometricAuthenticated: Boolean` including what the type honestly does not guarantee, session rules (a live session never covers STRONG_CONFIRMATION), lockout scoped honestly against the online path only, per-platform custody tiers, the recovery trade, what is refused by design, an honest landed-vs-not table, and slices A-1…A-9.
- `docs/COMPUTER_USE.md` — GUI automation and computer use: why it is the highest-consequence capability, the screen-as-untrusted-input threat and the rule that screen content is data and never instruction, consequence classes with their approval floors (SPEND and RECORD at STRONG_CONFIRMATION, CREDENTIAL refused outright), the verify/match/locate/confirm pre-flight loop, profile scoping of procedures and credentials, the multi-agent authority correction (the router never decides policy; no agent holds execution authority), audit and honest reversal, the injection-corpus testing approach, and slices C-1…C-9.
