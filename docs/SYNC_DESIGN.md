...
Date: 2026-08-11. Status: design + slice plan (S0–S7 + P1–P2). **Landed: S0 (DB substrate), S1 (engine core), S2 (identity/pairing/session crypto), S3 (LAN transport), S4 (relay), P1 (encrypted Quick Share protocol + discovery seam), P2 (Android BLE/WiFi-Direct proximity actual + Nearby Share UI + desktop CLI listener), W1 (Room-backed JournalStore + automatic loops on Android/Windows/macOS), capture slice 1 (graph + people record tables sync end-to-end).**Remaining: S7 files, P2-iOS (Multipeer Connectivity actual), and the rest of the syncable-table capture coverage.** **S6 (commands) landed**: targeted LOG-kind `commands` entries (`to:<peer>` inbox / `ack:` outbox, §6) — sender surfaces are the Android Sync screen's per-peer Send-command dialog and the desktop CLI `sync send`; intermediates relay and never execute (rule 11); the target's `CommandDispatcher` validates ttl + per-peer allowlist (`sync:peerperm:`) + schema, maps the class to a typed action, and runs the SAME policy spine as any agent-origin action (`PolicyEngine.evaluate(action, ActionOrigin.AGENT)` — rule 10), then acks `executed|refused|expired`; every evaluation lands in the policy audit trail. Deviations from §6/§13 (all deliberate, all tracked): payloads are inline JSON args, not `payloadRef` blob-store refs (no command blob store yet — §7); executors exist for `open_app` + `run_shell` only, other classes refuse with an explicit `execution-not-wired` ack (the full executor leg is Track M). Closed deviations: commands are now per-entry Ed25519-signed by the sender and verified against the paired peer's stored public key before TTL/allowlist/policy (forged or unknown senders get a `bad-signature` ack — `CommandSigning` in shared/sync); the desktop now runs its own `DesktopCommandDispatcher` (platform:windows:frontend, registered into `DesktopSync` — same validation order, `open_app` through the launch ladder, `run_shell` through the bounded shell capability); command history ships as a Sync-screen section (Android) + `sync history` (desktop CLI) reading straight from the journal. Desktop-originated settings capture (`captureSettings`) is gated by the "Settings & preferences" category toggle (Fix E). **Capture coverage extended (item 1)**: graph edges (`GraphStore` → `edges`, name-keyed), app usage (`AppScanner` → `app_records`), app capability links (`AppScanner` → `app_capability_links`, LWW per package+capability), trigger rules (`TriggerEngine` → `trigger_rules`, tombstones) — capture + materialize on Android AND desktop (desktop also materializes syncable `kv_store`). **Continuous listening (item 7)**: `SyncForegroundService` (dataSync FGS, START_STICKY) keeps the transport up; `SyncEngine` shares one cycle between it and the 15-min `SyncWorker` (worker behavior + status strings preserved verbatim). **Hierarchical agent memory (schema v14, docs/MEMORY_DESIGN.md)**: three synced layers join the mesh — `episodes` (episodic memory with outcome + lesson — collective learning), `handoffs` (shared-write artifacts + pointers), `library_entries` (the shared read-only Global Library behind the PENDING_APPROVAL human-in-the-loop gate, with background `distill()` conflict resolution) — plus two deliberately local tables (`agent_scratchpad` L2 private/TTL, `work_log` zero-duplication). Full producer surface (Android `AgentMemory` + Agent Memory screen; desktop CLI `sync episodes|library|know|approve|lesson|handoff|ack`) and materialize handlers on both platforms. **Desktop-originated capture (item 8)**: goals + policy saves journal `syncable:desktop-goals` / `syncable:desktop-policy` — peers store them; goal/policy REHYDRATION on the peer (incl. the §5 RUNNING→PENDING rule) is not implemented yet. **Relay deploy-shaped**: `relay/Dockerfile` + README (hosting deploy remains a platform action).S5 (unpair revocation) landed**: `peer_trust` journal records on pair/unpair (Android + desktops), re-applied via materialize; the relay's REG is now authoritative (resets the grant allowlist — the relay-side revocation mechanism, test-verified). **S5 settings surface (per-category toggles + per-peer command allowlists) landed**: `sync:cat:<table>` / `sync:peerperm:<deviceId>` local kv policy, gated at capture + materialize (trust always flows), with UI on the Android Sync screen + macosApp Sync screen + the desktop CLI (`sync categories` / `sync category` / `sync perms` / `sync perm`). Per-peer allowlists are stored + editable now and enforced by S6 command dispatch (design sequencing). **S4 (relay) now covers Android too**: the relay transport moved to jvmAndroidMain behind a `WsClient` seam (java.net.http on JVM, OkHttp on Android) and is wired into the Android `SyncWorker` relay phase + the SyncScreen relay URL field — WAN sync works wherever a relay is reachable. **Deploying the relay:** `relay/` is a bare Node service, already deploy-shaped (binds 0.0.0.0, reads `PORT`; install `npm --prefix relay install`, start `node relay/server.js` — see its header). The platform deploy CLIs are not available in the Linux sandbox, so the actual hosting deploy is a platform-side action; decide hosting per §14.2. **Track I (iOS) status:** audit + driver research done — `shared/sync` + `shared/database` commonMain verified platform-free (zero java./android./platform imports); Room 2.7.0-alpha13 KMP officially supports iOS (bundled sqlite driver); the remaining work — declaring the iOS targets, the 4 iosMain `actual`s (Keychain/CommonCrypto/CryptoKit-shim + Network.framework/POSIX transport + CoreBluetooth proximity), the `shared/database` iOS builder, and the `platform/ios/frontend` shell — **requires a Mac + Xcode and cannot be compiled or verified in the Linux sandbox**; exact steps in §15.
This document specifies how the four Newax Aegis devices — **I** (iOS), **A**
...
copy of the Newax Aegis data (memory fabric + files + settings + commands),
...
4. **`shared/database`: iOS target + builder.** Add the three iOS targets +
   `iosMain`, and a `NewaxDatabaseBuilder` actual over
