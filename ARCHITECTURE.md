...
├── platform/macos/         macOS adapters — planned (4-way track split, Phase 0 scaffolding; Track M body): FILES/PROCESSES/SHELL via JVM stdlib, DESKTOP via AXUIElement behind a seam, SECRETS via Keychain, SYSTEM
└── platform/ios/           iOS adapters — planned (4-way track split, Phase 0 scaffolding; Track I body): FILES via NSFileManager, SECRETS via Keychain (SecItem*), SYSTEM via UIDevice/NSProcessInfo; DESKTOP/PROCESSES/SHELL = NOT_SUPPORTED (sandbox)
```
