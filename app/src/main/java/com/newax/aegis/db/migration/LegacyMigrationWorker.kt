...
class LegacyMigrationWorker(ctx: Context, params: WorkerParameters) : Worker(ctx, params) {
    ...
    private fun migratePersonFacts(db: AegisDatabase, memory: EncryptedMemory) {
        ...
        for (name in names) {
            ...
            val inserted = db.personDao().insertIfAbsent(PersonEntity(name = name))
            val personId = if (inserted > 0L) inserted else db.personDao().idForName(name) ?: continue
    ...
    private fun migratePersonMentions(db: AegisDatabase, memory: EncryptedMemory) {
        ...
        for (name in names) {
            ...
            val resolved = db.personDao().idForName(name)
                ?: run {
                    val id = db.personDao().insertIfAbsent(PersonEntity(name = name))
                    if (id > 0L) id else db.personDao().idForName(name)
                }
            val personId = resolved ?: continue
