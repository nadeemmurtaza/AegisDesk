...
class EmbeddingIndexWorker(ctx: Context, params: WorkerParameters) : Worker(ctx, params) {
    ...
    override fun doWork(): Result {
        ...
        val indexed   = db.embeddingDao().getSourceIds(VectorStore.TYPE_FACT).toSet()
        val allIds    = db.personFactDao().getAllIds()
        ...
        val batch = db.personFactDao().getByIds(unindexed.take(BATCH_SIZE))
        for (fact in batch) {
    ...
    companion object {
        ...
        fun isNeeded(db: AegisDatabase): Boolean =
