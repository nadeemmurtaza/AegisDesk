...
object AutomationSettings {
    ...
    fun isEnabled(toggle: AutomationToggle): Boolean =
        prefs?.getBoolean(toggle.key, false) == true

    fun setEnabled(toggle: AutomationToggle, enabled: Boolean) {
        prefs?.edit()?.putBoolean(toggle.key, enabled)?.apply()
    }

    fun enableAllNonSensitive() {
        val editor = prefs?.edit() ?: return
        AutomationToggle.entries.filter { !it.sensitive }.forEach { editor.putBoolean(it.key, true) }
        editor.apply()
    }

    fun disableAll() {
        val editor = prefs?.edit() ?: return
        AutomationToggle.entries.forEach { editor.putBoolean(it.key, false) }
        editor.apply()
    }
    ...
    fun importAll(map: Map<String, Boolean>) {
        val editor = prefs?.edit() ?: return
        editor.clear()
        map.forEach { (k, v) -> editor.putBoolean(k, v) }
        editor.apply()
    }
