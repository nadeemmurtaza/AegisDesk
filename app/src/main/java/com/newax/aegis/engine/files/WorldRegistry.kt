...
object WorldRegistry {
    ...
    fun findSimilarImages(db: AegisDatabase, queryHash: String, maxDistance: Int = 10, limit: Int = 10): List<FileObject> {
        ...
        return similar.mapNotNull { (id, _) -> db.fileDao().byId(id) }
    }

    fun findSimilarImagesById(db: AegisDatabase, fileId: Long, limit: Int = 10): List<FileObject> {
        val fo = db.fileDao().byId(fileId) ?: return emptyList()
        if (fo.pHash.isBlank()) return emptyList()
        return findSimilarImages(db, fo.pHash, limit = limit)
    }

    // ── Duplicate report ──────────────────────────────────────────────────────

    fun duplicateSummary(db: AegisDatabase): String {
        val count = db.fileDao().duplicateCount()
        val total = db.fileDao().totalFiles()
        return "$count duplicate files found out of $total total."
    }
}
