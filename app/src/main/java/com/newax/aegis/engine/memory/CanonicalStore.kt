...
object CanonicalStore {
    ...
    fun write(
        ...
        val existing = db.memoryRecordDao().findByHash(hash)
        if (existing != null) {
            db.memoryRecordDao().bumpImportance(existing.id, maxOf(existing.importance, importance), now)
            return existing.id
        }
        val recordId = db.memoryRecordDao().insert(
            MemoryRecord(
                type        = type,
                content     = content,
                category    = category,
        ...
        db.memoryRecordDao().updateEmbeddingId(recordId, embId)
    ...
    fun invalidate(db: AegisDatabase, recordId: Long) {
        db.memoryRecordDao().invalidate(recordId, System.currentTimeMillis())
    }
