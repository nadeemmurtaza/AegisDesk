...
object AppIntelligence {
    ...
    fun resolve(
        ...
        val candidates = if (packageHint != null) listOf(packageHint)
                         else runBlocking { db.appRegistryDao().packagesByCapability(capability.name) }
        ...
        val link = runBlocking { db.appRegistryDao().linkFor(pkg, capability.name) }

        // 2. Deep link
        val deepLinkPattern = link?.deepLinkPattern
        if (deepLinkPattern != null) {
            val uri = buildDeepLink(deepLinkPattern, capability, extras)
        ...
        val proc = runBlocking { db.appRegistryDao().bestProcedure(pkg, capability.name) }
