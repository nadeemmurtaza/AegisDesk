        ...
        for (name in plan.entityNames.take(2)) {
            ...
            hops.drop(1).take(8).forEach { node ->
                node.edges.take(3).forEach { edge ->
                    val pred = kotlinx.coroutines.runBlocking { db.graphDao().predicateById(edge.predicateId) }?.name ?: return@forEach
                    val obj  = edge.objectId?.let { kotlinx.coroutines.runBlocking { db.graphDao().entityById(it) }?.canonicalName } ?: edge.objectValue ?: "?"
