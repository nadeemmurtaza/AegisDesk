...
class AegisAccessibilityService : AccessibilityService(), SemanticAutomation {
    ...
    fun execute(action: ProposedAction): Boolean {
        if (action !is ProposedAction.OpenApp && 
            action !is ProposedAction.Home && 
            action !is ProposedAction.Recents && 
            action !is ProposedAction.Back && 
            action !is ProposedAction.ToggleConnectivity) {
            val pkg = currentPackage.ifBlank { "unknown" }
            if (!com.newax.aegis.engine.apps.AppPermissionManager.isActionAllowed(pkg)) {
                return false
            }
        }
        return when (action) {
