...
class PolicyAuditStoreTest {
    ...
    fun `actionClassBreakdown groups counts and sorts by human approval pressure`() {
        ...
        // Send prompts twice; RunScript prompts once; DeleteFile never prompts
        // (its DENY is counted separately, not as a human prompt) so it sorts
        // after the prompting classes, deterministically by needsHuman/total/name.
        assertEquals(listOf("Send", "RunScript", "DeleteFile"), breakdown.map { it.actionClass })
        ...
        assertEquals(0, delete.needsHuman)
