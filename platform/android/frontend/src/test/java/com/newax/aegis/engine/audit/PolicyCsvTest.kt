...
class PolicyCsvTest {
    ...
    fun `csv renders a full record row`() {
        ...
        assertTrue(row.contains("CRITICAL"))
    ...
    fun `csv escapes commas quotes and newlines in reason`() {
        ...
        // Split on the record terminator, not lineSequence(): the embedded
        // newline inside the quoted field is not a record boundary.
        val row = csv.split("\r\n")[1]
    ...
    fun `csv preserves input order`() {
        ...
        val rows = csv.lineSequence().filter { it.isNotEmpty() }.toList().drop(1)
